...
export default function EnrollmentCard({
  ...
  // --- BILLION-DOLLAR ENROLLMENT LOGIC (STRANGLER FIG ARCHITECTURE) ---
  const handleEnroll = async () => {
    ...
    const toastId = toast.loading(
      isPaid
        ? "Securing data with Aegis Engine..."
        : "Processing your free enrollment...",
    );

    try {
      // Define our two backend services
      const mockServerUrl =
        process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:4000";
      const aegisEngineUrl = "http://localhost:5000";
      ...
      // ==========================================
      // SCENARIO A: PAID COURSE (Aegis + PhonePe)
      // ==========================================
      if (isPaid) {
        // Send sensitive data to the Microservice Vault
        const checkoutRes = await axios.post(`${aegisEngineUrl}/api/checkout`, {
          userId: userId,
          courseId: course.id,
          amount: course.price,
          customerName: currentUser.name || "Student",
          email: currentUser.email || "student@bhashasync.com",
        });

        const data = checkoutRes.data;

        if (data.success && data.redirectUrl) {
          toast.success("Data secured! Redirecting to Gateway...", {
            id: toastId,
          });
          ...
          // If backend returns a 200 OK but success is false, throw the specific backend message
          throw new Error(
            data.message || "Aegis Engine failed to secure transaction.",
          );
        }
      }

      // ==========================================
      // SCENARIO B: FREE COURSE (Standard json-server)
      // ==========================================
      ...
      const freshCourseRes = await axios.get(
        `${mockServerUrl}/courses/${course.id}`,
      );
      const currentStudents = freshCourseRes.data.students || 0;

      await axios.patch(`${mockServerUrl}/courses/${course.id}`, {
      ...
      // EXPLICIT FAILURE HANDLING: Extract the exact error from Axios or use a fallback
      const errorMessage =
        error.response?.data?.message || // Message sent from your Express res.status(500).json(...)
        error.message || // Message from standard JavaScript errors
        "Failed to connect to secure engine. Please try again.";

      // Display the specific error in the UI
  ...
  const executeRating = async (selectedStars) => {
    ...
    const toastId = toast.loading(
      userRating > 0 ? "Updating your review..." : "Submitting your review...",
    );
    ...
    try {
      ...
      const currentReviewCount =
        currentRating === 0 ? 0 : freshCourse.reviewCount || 0;
      ...
      if (userRating === 0) {
        ...
        newAverageRating =
          currentReviewCount === 0
            ? selectedStars
            : (currentRating * currentReviewCount + selectedStars) /
              newReviewCount;
      } else {
        ...
        if (currentReviewCount <= 1) {
          ...
          newAverageRating =
            (currentRating * currentReviewCount - userRating + selectedStars) /
            currentReviewCount;
        }
      }
      ...
      toast.success(`Review successfully set to ${selectedStars} stars!`, {
        id: toastId,
      });
    ...
    <>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#161a23] w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl dark:shadow-blue-900/20 transform transition-all border border-gray-100 dark:border-white/10 animate-fade-in">
            ...
            <p className="text-center text-gray-500 dark:text-gray-400 font-medium mb-8">
              You previously rated this course{" "}
              <span className="font-bold text-gray-900 dark:text-white">
                {userRating} stars
              </span>
              . Are you sure you want to change it to{" "}
              <span className="font-bold text-yellow-500">
                {confirmModal.pendingRating} stars
              </span>
              ?
            </p>
      ...
      <div className="lg:sticky lg:top-28 bg-white dark:bg-[#161a23] rounded-[2.5rem] p-8 border border-gray-100 dark:border-white/10 shadow-2xl dark:shadow-blue-900/10 overflow-hidden relative transition-colors duration-300">
        <div className="w-full aspect-video rounded-2xl overflow-hidden mb-6 border border-gray-100 dark:border-white/5 relative group">
          <img
            src={`https://picsum.photos/seed/${course.id}x/600/400`}
          ...
          {isEnrolled ? (
            <div className="animate-fade-in">
              ...
              <Link
                ...
                className="flex justify-center items-center gap-2 w-full py-4 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-black text-lg shadow-lg shadow-green-600/30 transition-all hover:-translate-y-1 active:scale-95"
              >
                Start Learning Now
                <svg
                  className="w-5 h-5"
            ...
            <div className="animate-fade-in">
              ...
              <button
                ...
                className={`w-full py-4 rounded-2xl font-black text-lg shadow-lg transition-all flex justify-center items-center gap-2 ${isProcessing ? "bg-blue-400 cursor-not-allowed text-white shadow-none" : "bg-blue-600 hover:bg-blue-700 text-white shadow-blue-600/30 hover:-translate-y-1 active:scale-95"}`}
              >
                {isProcessing
                  ? "Processing..."
                  : course.price && course.price > 0
          ...
          <div className="pt-6 border-t border-gray-100 dark:border-white/10 space-y-4">
            ...
            <ul className="space-y-3">
              {[
                {
                  icon: "🎥",
                  text: `${course.modules?.length || 0} High-quality videos`,
                },
                ...
                <li
                  key={i}
                  className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 font-medium"
                >
                  <span>{item.icon}</span>
                  {item.text}
                </li>
